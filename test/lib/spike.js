    ...
    var assert = {
        ...
        isEmpty: function (assertionToCheck) {
            if (assertionToCheck.length === 0 || !assertionToCheck) {
                passed({type: "Passed test", details: {assertion: assertionToCheck, expected: 'to be empty'}});
            }
            else {
                failed({type: "Failed test", details: {assertion: assertionToCheck, expected: 'to be empty'}});
        ...
        htmlContainsText: function (html, htmlTag, text) {
            ...
            if (found) {
                passed({type: "Passed test", details: {actual: actual, expected: text}});
                return;
            }
            else {
